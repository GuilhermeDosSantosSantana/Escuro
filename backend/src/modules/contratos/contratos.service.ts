...
import { validateCreateContrato, validatePatchContrato, type CriarContratoInput } from "../../shared/validators/contrato.validator.js";
...
async function assertPlanoAtivo(idPlano: string) {
...
async function assertContratoExiste(idContrato: string) {
...
async function assertDuplicidadeAtiva(msisdn?: string, iccid?: string, ignoreIdContrato?: string) {
  if (msisdn) {
...
export async function criarContrato(body: unknown) {
  ...
  const contrato = await prisma.$transaction(async (tx: Parameters<Parameters<typeof prisma.$transaction>[0]>[0]) => {
...
export async function listarContratos(query: Record<string, unknown>) {
...
export async function buscarContrato(idContrato: string) {
  return assertContratoExiste(idContrato);
}

export async function atualizarContratoPatch(idContrato: string, body: unknown) {
  ...
  if (typeof data.msisdn === "string" || typeof data.iccid === "string") {
    await assertDuplicidadeAtiva(
      typeof data.msisdn === "string" ? data.msisdn : undefined,
...
export async function substituirContrato(idContrato: string, body: unknown) {
...
export async function encerrarContrato(idContrato: string) {
...
function validarDatas(dataInclusao: Date, dataEncerramento?: Date | null) {
